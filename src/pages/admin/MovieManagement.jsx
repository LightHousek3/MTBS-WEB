...
const MovieManagement = () => {
  ...
  const [isImageMarkedForDeletion, setIsImageMarkedForDeletion] =
    useState(false);
  const [isTrailerMarkedForDeletion, setIsTrailerMarkedForDeletion] =
    useState(false);
  ...
  const handleModalOk = async (e) => {
    ...
    try {
      ...
      form.setFields(
        Object.values(fieldNameMap).map((name) => ({ name, errors: [] })),
      );
      ...
      if (isEditMode && editingMovie) {
        ...
        if (
          Number(movieData.duration) !== Number(originalMovieData?.duration)
        ) {
          changedFields.duration = movieData.duration;
        }
        if (
          movieData.releaseDate !==
          normalizeDateForInput(originalMovieData?.releaseDate)
        ) {
          changedFields.releaseDate = movieData.releaseDate;
        }
        if (
          movieData.endDate !==
          normalizeDateForInput(originalMovieData?.endDate)
        ) {
        ...
        const hasTrailerChanged =
          trailerFile !== null || isTrailerMarkedForDeletion;
  ...
  const handleEdit = (movie) => {
    ...
    setImageUploadProgress(0);
    setTrailerUploadProgress(0);
    setUploadedImageAsset(
      movie.image && typeof movie.image === "object" ? movie.image : null,
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (type) => (
        <Tag color={type === "2D" ? "green" : "purple"}>
          {type === "2D" ? "2D" : "3D"}
        </Tag>
      ),
    },
    {
      ...
      render: (ageRating) => {
        ...
        return ageRating ? (
          <Tag color={colorMap[ageRating]}>{ageRating}</Tag>
        ) : (
          "-"
        );
      },
            ...
            <div className="!bg-white p-6 rounded-lg border border-blue-200">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center gap-2">
                  <span className="text-gray-600 font-medium">Loại phim:</span>
                  <Tag color={viewingMovie.type === "2D" ? "green" : "purple"}>
