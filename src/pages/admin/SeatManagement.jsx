...
const getTheaterName = (screen, fallback = '-') =>
    screen?.theater?.name || screen?.theaterId?.name || fallback;
...
const SeatManagement = () => {
    ...
    const screenOptions = screens.map((screen) => ({
        ...
        label: `${screen.name} - ${getTheaterName(screen, 'Chưa có rạp')}`,
    }));
    ...
    const applySeatResponse = useCallback((response, page, limit) => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            setLoading(true);
            if (editingSeat) {
                await seatAPI.updateSeat(editingSeat.id || editingSeat._id, {
                    ...
                    viewingSeat && (
                        <Descriptions bordered column={2} size="small">
                            ...
                            <Descriptions.Item label="Rạp">
                                {getTheaterName(getScreenById(viewingSeat.screenId))}
                            </Descriptions.Item>
